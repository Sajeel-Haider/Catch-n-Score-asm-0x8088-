...
scrollAndSpawnCheck:
    ...
    jne dontScroll
        ...
        ;new logic store x cor of spawn and check if its y coordinate reaches 14h or
        ; something and also posOfPikaxe is same as cordinate then it collides
        ;calculate 
        call    detectPosOfSpwnObj ;returns in si
        call    detectPosAbovePickaxe  ;returns in di
        ; mov     ax,     0xb800
        ; mov     es,     ax
        ; mov    word[es:di],    0x0720
        ; mov    word [es:si],    0x0220
        ; cld
        ; lodsw

        cmp     si,     di
        jne     noObject  
        cmp     word[posSpwnObjX],  word[posOfPickaxe]
        jne     noObject
        cmp     word[posSpwnObjY],  
...
spawnObject:
    ...
    push    ax
    mov     word[posSpwnObjX],   ax
    mov     ax,     4 ; row can be randomly selected using random function X
    push    ax
    mov     word[posSpwnObjY],  ax
    call    minPointShape
